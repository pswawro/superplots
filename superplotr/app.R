...
server <- function(input, output, session) {
    ...
    superplot <- reactive({
        if(input$geom == "Beeswarm") {
            gg() + geom_beeswarm(cex = input$cex, alpha = 0.6) +
                geom_beeswarm(data = data_summary(), aes(x = condition, y = mean, fill = rep, shape = rep), size = 5) +
    ...
    observe(updateSliderInput(session, "y_scale", min = 0,
                              max = round(max(data()$val) * 1.1, digits = 2), value = c(0, round(max(data()$val) * 1.1, digits = 2))
    ))
